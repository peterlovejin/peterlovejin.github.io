...
full$date=str_sub(full$date,5,10)
yes$date=str_sub(yes$date,5,10)
...
for (x in j) {
print(aa2 %>% filter(stake==x) %>% ggplot(aes(date,total))+geom_bar(aes(fill=ifelse(total>0,"blue","red")),stat="identity",position="dodge")+labs(title=x,x="",y="")+theme(legend.position = "none")+facet_wrap(name~.,ncol=7))
}
