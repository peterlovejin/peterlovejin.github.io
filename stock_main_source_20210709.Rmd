...
full=NULL
  ...
  my=c("2390","6180","1570","2727","3679","6207","2841","5410","2929" ,"1570","8112","2363")
...
print(full1 %>%filter(stake==x)  %>% ggplot()+geom_bar(aes(reorder(name,total),total,fill=ifelse(total>0,"blue","red")),stat="identity",position = "dodge")+geom_hline(yintercept=0)+theme(axis.text.x=element_text(angle = 0,size=8))+geom_hline(yintercept = 0)+labs(title=paste(x,"@",dd),y="",x="") +theme(legend.position = "none")+coord_flip()) }
...
for (x in j) {
print(aa2 %>% filter(stake==x) %>% ggplot(aes(as.character(date),total))+geom_bar(aes(fill=ifelse(total>0,"blue","red")),stat="identity",position="dodge")+geom_hline(yintercept=0)+labs(title=x,x="",y="") +theme(axis.text.x = element_text(angle=20,size = 7),legend.position = "none")+facet_wrap(name~.,ncol=7))
}
...
print(aa3 %>%filter(stake==x)  %>% ggplot()+geom_bar(aes(reorder(name,t),t,fill=ifelse(t>0,"blue","red")),stat="identity",position = "dodge")+geom_hline(yintercept=0)+theme(axis.text.x=element_text(angle = 20,size=8))+geom_hline(yintercept = 0) +theme(legend.position = "none")+coord_flip()+labs(title=paste(x,"@",start_d,"~",end_d),y="",x="")) }
